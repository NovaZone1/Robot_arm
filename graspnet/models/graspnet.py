...
# Lazy training-only imports to avoid knn_pytorch dependency at inference time.
from loss import get_loss
from loss_utils import GRASP_MAX_WIDTH, GRASP_MAX_TOLERANCE, batch_viewpoint_params_to_matrix

_batch_vp_to_mat = batch_viewpoint_params_to_matrix

def _get_training_imports():
    from label_generation import process_grasp_labels, match_grasp_view_and_label
    return process_grasp_labels, match_grasp_view_and_label
...
class GraspNetStage2(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if self.is_training:
            _, match_grasp_view_and_label = _get_training_imports()
...
class GraspNet(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if self.is_training:
            process_grasp_labels, _ = _get_training_imports()
