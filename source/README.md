...
- YOLOv8-seg + GraspNet + Open3D + pyrealsense2 运行在 `piper` conda 环境
...
- 当前候选策略已切到宽松模式：不再用 strict top-down 强过滤候选，感知侧会合并实例点云候选和全场景 mask 过滤候选，分割为 0 时会用全场景 grasp 兜底，执行前再由 robot validation 逐个姿态变体验证可达性
