...
比赛瓶子真机默认执行策略是 `center_horizontal`：YOLOv8 分割得到瓶子几何中心，保留规划层 Z/人工补偿后，以已验证的水平夹爪姿态执行“观察 -> 感知 -> 安全高度横移 -> 在瓶前 80 mm 处下降到抓取高度 -> 低速水平接近 -> 闭爪 -> 水平撤离 -> 抬升”。夹爪必须先产生明确闭合运动并连续稳定，且最终开口须位于 `12..65 mm` 才视为夹住瓶子；否则撤离后终止，不再带着张开的夹爪继续推瓶。物块仍使用原有顶部垂直抓取。Dashboard 默认勾选“瓶子中心水平抓取”，点“直接抓取”即可一次触发；取消勾选时回退到兼容的 `safe_top_down` 路径。
