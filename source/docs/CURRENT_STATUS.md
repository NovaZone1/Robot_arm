...
2026-07-27 六类物品识别与对应盒放置：

- `config/item_catalog.yaml` 已登记黄/红/蓝物块与橙/深/绿色饮料瓶及用户提供的六张参考图。
- 抓取前目标会解析成 catalog item；物块使用 HSV 颜色实例，瓶子在 YOLO bottle 实例上
  增加液体颜色过滤，指定目标未命中时不允许全场景兜底。
- 抓取目标单向扫描以 2026-08-08 操作员确认的两张标准帧为准：瓶子中心
  `(0.598, 0.485)`，物块中心 `(0.606, 0.619)`；采用水平 `±0.08`、垂直 `±0.15`
  的宽松范围。目标出现但未居中时改用 7 cm 细步。缺少真实实例中心的 scene fallback
  候选不会触发停车或抓取。
- 通过底盘扫描抓到任一目录目标（瓶子或物块）后，顺序固定为
  `close + retreat -> (底盘连续前进 1.5 m || placement_observation)`；两项并行执行。
  放置松爪并退出盒体后，底盘再连续前进 `1.5 m`。
- 新增 `/vision_worker/match_item_label`：一次识别后侧竖直面上的全部六个盒标，按观察
  画面从左到右生成 `slot_index=0..5`；盒子排列顺序可以变化，缺少/重复任一标识即拒绝。
- 新增 `/robot_executor/execute_place_plan` 与 `PlacePlan`：放置前检查盒尺寸
  `180 x 132 x 87 mm`、标识校验、垂直进退距离和三个路点的 MoveIt IK；先 dry-run，
  再执行“到盒上方 -> 低速下降 -> 松爪 -> 垂直退出 -> 可选 Home”。
- Dashboard 已增加六类目标和“识别对应盒标并放置”开关。
- 用户示意图已离线验证，当前顺序可正确恢复为
  `blue, red, yellow, orange, dark, green`，六个目标槽位均可解析。
- 已确认盒子长边 `180 mm` 沿桌长紧密排列，整排约 `1080 mm`。默认启用 RGB-D 动态
  定位：对纸质标识取中值深度、转换到 `base_link`、校验五段相邻间距为
  `180±20 mm`，再由后壁向相机方向偏移 `66 mm` 得到盒几何中心。透明盒壁不参与测距。
- 深度无效、六标识不完整、行间距异常或无法确定盒内方向时 fail closed，不回退盲放；
  静态六槽位/左右端点仅作为显式关闭动态定位后的校准路径。
- 软件接口、消息与保护逻辑已构建；真实放置尚未验收。2026-07-30 已确认较低观察位
  原左侧观察位在桌子移到机械臂右侧后按 Y/yaw 镜像为
  `[0.0, -35.5, 491.1, 180.0, 67.77, 89.97]`，诊断关节参考同步为
  `[-1.571774176, 0.652510264, -1.397299288, 0.0, 1.218672728, 0.0] rad`。
  原 `Z≈542 mm / pitch≈80 deg` 高观察位会在部分关节分支触发 `ANGLE_LIMIT`，停止用于
  自动任务。各物品 `release_rpy_deg/release_offset_mm` 仍需标定；在此之前所有
  `placement.enabled=false`，自动放置会被明确拒绝。
- Dashboard 新增“扫描放置区（无运动）”：读取当前位姿并调用 capture + label/depth
  localization，结果保存到 `ros_ws/viz/placement_scan/`；不调用任何运动计划或夹爪命令。
- 盒标识别已改为 `YOLO bottle shape + liquid color + block color/contour +
  edge/Lab template` 融合；不再因颜色分支先检出部分结果而跳过形状/模板分支。
- Scout 预扫描从盒排左侧附近开始、沿前进方向每 `0.15 m` 停车采图；当前最远
  `1.50 m` / 最多 24 个视角，发现目标后以 `0.07 m` 细步居中；不再执行
  `中间 -> 前 -> 中间 -> 后 -> 中间` 往返扫描，失败时停车并停留在当前位置。
- 单向扫描只融合重叠画面中的二维标签左右关系，用拓扑排序恢复随机标签对应的
  `slot_index=0..5`；透明盒和纸标深度完全不参与槽位顺序。六类标签缺失、重复关系
  冲突或视角间无重叠导致顺序不唯一时继续 fail closed。
- 2026-07-30 实扫发现一个 `confidence=0.447` 的深色瓶误检会与后续多个清晰视角形成
  硬循环；现改为按标签对中较低置信度进行多视角加权投票，净权重不足或接近平票仍
  拒绝。该轮 10 视角已离线重放为
  `blue_block -> green_bottle -> yellow_block -> dark_bottle -> red_block -> orange_bottle`。
- 当前单向扫描仅完成“标签 -> 固定槽位编号”验证，不会解锁自动放置。下一阶段仍需
  标定“对准后机械臂固定释放位姿”；在该标定完成前 Dashboard 会保持
  `use_cached_multiview_box_map=false`。
- Dashboard 已增加“对准目标盒”：仅接受成功且未过期的单向扫描，并验证 Scout 仍在
  扫描终点；从目标标签的多帧观测中选择最接近图像中心的视角，按单段不超过 `0.40 m`
  分段返回到对应里程计位置。该动作只移动底盘，不移动机械臂、不打开夹爪，也不会解锁
  自动放置。
- 330 ml 瓶子需要横放或斜放进 `18 x 13.2 x 8.7 cm` 盒，最终姿态与净空必须以实物
  试放和低速空载轨迹为准，不能直接套用物块的竖直释放姿态。
...
- 当前右侧桌面观察位已固化为 `[0.0, -35.5, 491.1, 180.0, 67.77, 89.97]`（`mm/deg`）。
- 2026-08-09 右侧抓取验证中，保持观察位 `Z≈491 mm` 横移到目标上方会在 `topdown_lateral` 返回 MoveIt `NO_IK_SOLUTION(-31)`。执行器现改为先在原地到达 `max(target_z + approach_height, min_safe_z)`，再调腕和横移；本次红块几何对应过渡高度约 `364 mm`。
...
2026-07-27 夹爪张开与观察位超时修正：

- Dashboard 真机栈使用完整物理开度命令，Piper `gripper_val_mutiple` 默认改为 `1`；
  RViz 双指半行程换算只由反馈 relay 处理。
- 张开等待按“反馈开度至少达到目标减容差”判定，机械限位处反馈略大于目标不再误报超时。
- 5% 低速移动到观察位时，pipeline 服务等待上限提高到 `45 s`，不再早于 executor
  的实际运动超时返回。
